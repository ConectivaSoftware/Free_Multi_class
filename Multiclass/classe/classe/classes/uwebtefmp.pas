...
procedure SA_DesativarBTCancelar;
...
//------------------------------------------------------------------------------
//   Destivar botão botão cancelar
//------------------------------------------------------------------------------
procedure SA_DesativarBTCancelar;
begin
   frmwebtef.pnutil.Visible := false;
end;
...
begin
   ...
   if length(mensagem)>65 then
      begin
         ...
         frmwebtef.pnaviso.Font.Color := clBlack;
      end
   ...
   if ativar then
      begin
         ...
         if frmwebtef.mensagem<>'' then
           begin
              if pos(#13,frmwebtef.mensagem)>0 then
                 begin
                    frmwebtef.aviso_linhas_jv.Caption   := frmwebtef.mensagem;
                    frmwebtef.aviso_linhas_jv.Align     := alClient;
                    ...
                    if length(frmwebtef.mensagem)>60 then
                       frmwebtef.aviso_linhas_jv.Font.Size := 14
                    else if length(frmwebtef.mensagem)>30 then
                       frmwebtef.aviso_linhas_jv.Font.Size := 20
                    else
                       frmwebtef.aviso_linhas_jv.Font.Size := 20
                 end
              else
                 begin
                    ...
                    if length(frmwebtef.mensagem)>60 then
                       frmwebtef.pnmensagem.Font.Size := 14
                    else if length(frmwebtef.mensagem)>30 then
         ...
         frmwebtef.lblinha1.Width     := frmwebtef.pnmensagem.Width-4;
         frmwebtef.lblinha1.Alignment := taCenter;
         ...
         frmwebtef.lblinha2.Width     := frmwebtef.pnmensagem.Width-4;
         frmwebtef.lblinha2.Alignment := taCenter;
...
begin
   ...
   frmwebtef.btcancelar.Py           := 10; //frmwebtef.placa_util.Top  + 10;
   frmwebtef.btcancelar.Px           := (frmwebtef.pnutil.Left + frmwebtef.pnutil.Width) - 370; //(frmwebtef.placa_util.Left + frmwebtef.placa_util.Width) - 370;
   frmwebtef.btcancelar.Altura       := frmwebtef.pnutil.Height - 20;                      //frmwebtef.placa_util.Height - 20;
   ...
   frmwebtef.btcancelar.Pai          := frmwebtef.pnutil;
      ...
      begin
         ...
         try
            case TipoTef of
              ...
              tpVEROSmartTEF        : logomp.Picture.LoadFromFile(GetCurrentDir+'\icones\VEROSmart.bmp');   // VERO SMART;
              tpSTONESmartTEF       : logomp.Picture.LoadFromFile(GetCurrentDir+'\icones\Stone.bmp');       // STONE
         ...
         //---------------------------------------------------------------------
         case TipoTef of   // Selecionando o título apresentado durante a transação do TEF
           tpTEFELGIN          : titulo.Caption := 'TEF ELGIN';          // ELGIN
           tpTEFMultiPlus      : titulo.Caption := 'TEF MULTIPLUS';      // Multiplus Pay
           tpMKMPix            : titulo.Caption := 'MKM API PIX';        // Pix API MKM
           tpVEROSmartTEF      : titulo.Caption := 'VERO Smart TEF';     // VERO SMART
           tpSTONESmartTEF     : titulo.Caption := 'STONE Smart TEF';    // STONE Smart
