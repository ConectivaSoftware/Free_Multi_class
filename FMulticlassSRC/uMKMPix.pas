...
type
   ...
   TMKMPix = class
     private
        ...
        LEmitente     : TEmitente;                // Configurações do Emitente - Recebedor
        //----------------------------------------------------------------------
        LChave                : string;
        LCNPJ                 : string;
        LHostMKMPIX           : string;
     ...
     public
        ...
        property Emitente           : TEmitente           read LEmitente      write LEmitente;      // Configurações do Emitente - Recebedor
        //----------------------------------------------------------------------
...
begin
   ...
   if SA_UsarPinpad then
      begin
         try
            pinpad := TKSPinpad.create(LConfigPinPad);
            qrsize := pinpad.TamanhoTela;
...
begin
  ...
  if SA_UsarPinpad then
     begin
        try
           pinpad := TKSPinpad.create(LConfigPinPad);
           pinpad.MostrarImagem(LConfigPinPad.PINPAD_Imagem);
           pinpad.Free;
        except on e:exception do
           begin
