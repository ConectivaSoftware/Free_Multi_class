...
Type
   ...
   TMulticlass = class
     Private
        //----------------------------------------------------------------------
        LMovifluxoToken       : string;
        LMovifluxoIDCliente   : string;
        LMovifluxoHomologacao : boolean;
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
        function SA_EncontrarEventoMultiplus(forma:string):TtpMultiplusFormaPgto;
     Public
        constructor create;
        destructor Destroy(); override;
        //----------------------------------------------------------------------
        property MovifluxoToken       : string read LMovifluxoToken        write LMovifluxoToken;
        property MovifluxoIDCliente   : string read LMovifluxoIDCliente    write LMovifluxoIDCliente;
        property MovifluxoHomologacao : boolean read LMovifluxoHomologacao write LMovifluxoHomologacao;
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
        function TransacionarMultiplus(valor: real; forma: string;  parcelas: integer;cnpj,CodigoLoja,pdv : string):TRetornoPagamentoTEF;
        //----------------------------------------------------------------------
...
begin
   if LMovifluxoToken<>'' then
      begin
         LConfig.ConfigMovifluxo.Token       := LMovifluxoToken;
         LConfig.ConfigMovifluxo.Homologacao := LMovifluxoHomologacao;
      end;
   if LMovifluxoIDCliente<>'' then
      LConfig.ConfigMovifluxo.IDCliente := LMovifluxoIDCliente;
...
function TMulticlass.SA_EncontrarEventoMultiplus(forma: string): TtpMultiplusFormaPgto;
var
   d : integer;
begin
   Result := tpMPlPerguntar;
   for d := 1 to length(FormasMultiplus) do
      if uppercase(FormasMultiplus[d-1])=uppercase(forma) then
         Result := TtpMultiplusFormaPgto(d-1);
end;
...
function TMulticlass.TransacionarMultiplus(valor: real; forma: string;  parcelas: integer;cnpj,CodigoLoja,pdv : string): TRetornoPagamentoTEF;
var
   Pagamento : TPagamentoFoma;
begin
   Pagamento.Achou          := true;
   Pagamento.Valor          := valor;
   Pagamento.Codigo         := 99;
   Pagamento.Forma          := forma;
   Pagamento.Evento         := tpKSEventoMultiplus;
   Pagamento.FormaTEF       := 0;
   Pagamento.FormaMultiplus := SA_EncontrarEventoMultiplus(forma);
   Pagamento.Parcelas       := parcelas;
   //---------------------------------------------------------------------------
   LConfig.ConfigTEFEMultiplus.CNPJ        := cnpj;
   LConfig.ConfigTEFEMultiplus.Loja        := CodigoLoja;
   LConfig.ConfigTEFEMultiplus.TerminalPDV := pdv;
   //---------------------------------------------------------------------------
   Result := SA_MultiPlusPagar(Pagamento);    // Transacionar com MULTIPLUS
   //---------------------------------------------------------------------------
end;
